...
void BQ25120A_ENTER_SHIP_MODE(uint8_t ship_mode_sel){
    ...
    if(ship_mode_sel == ENTER_SHIP_MODE){
       ...
       GPIO_Write_High(BQ_CD_Pin); //to enable active battery mode for ı2c and disable charge //To enter Ship Mode drive CD pin high
       HAL_Delay(5);
...
bool BQ25120A_Check_Valid_VinSource(GPIO_TypeDef* PG_PORT,uint32_t PG_pin_dio){
    ...
    bool input_state = GPIO_Read_Pin(BQ_PG_Pin);
...
void BQ25120A_I2C_WRITE(uint8_t slaveaddr, uint8_t regaddr, uint8_t data){
	...
	GPIO_Write_High(BQ_CD_Pin); //to enable active battery mode for ı2c and disable charge
	HAL_Delay(10);
	I2C_Write(slaveaddr,regaddr,1,(uint8_t *)&data,1);
	HAL_Delay(10);
	GPIO_Write_Low(BQ_CD_Pin); //to disable active battery mode for enable charge
...
uint8_t BQ25120A_I2C_READ(uint8_t regaddr){
    ...
    GPIO_Write_High(BQ_CD_Pin);//to enable active battery mode for ı2c and disable charge
    HAL_Delay(10);
    I2C_Read(ADDR_BQ25120A,regaddr,1,(uint8_t *)&rx_data,1);
    HAL_Delay(10);
    GPIO_Write_Low(BQ_CD_Pin); //to disable active battery mode for enable charge
