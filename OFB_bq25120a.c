...
static struct Comm_functions functions;


 STATE_BQ25120A Init_BQ25120A(struct Init_bq25120a init_bq25120a){

    functions = Init_bq25120a.Comm_functions;	

    BQ25120A_ENTER_SHIP_MODE(EXIT_SHIP_MODE);

    BQ25120A_Battery_Voltage_Control(5);

    BQ25120A_Charge_Control(init_bq25120a.fast_charge_curr, init_bq25120a.termination_curr); 

    BQ25120A_Set_SW_Voltage(init_bq25120a.enable_buck, init_bq25120a.buck_voltage_range, init_bq25120a.buck_voltage); 

    BQ25120A_Set_LDO(init_bq25120a.enable_ldo, init_bq25120a.ldo_voltage, init_bq25120a.mrreset_vin_vfg);
 
    BQ25120A_MR_Ctrl(init_bq25120a.push_button_cfg);

    BQ25120A_Ilim_Uvlo_Ctrl(NO_EFFECT_RESET, init_bq25120a.input_curr, init_bq25120a.buvlo_cfg);

    if(init_bq25120a.wdt_enable == WDT_RESET_ENABLE) BQ25120A_BatmonINT_Init(init_bq25120a.wdt_interval);
...
void BQ25120A_ENTER_SHIP_MODE(uint8_t ship_mode_sel){
    ...
    if(ship_mode_sel == ENTER_SHIP_MODE){
       ...
       functions.GPIO_Write_High(BQ_CD_Pin); //to enable active battery mode for ı2c and disable charge //To enter Ship Mode drive CD pin high
       HAL_Delay(5);
...
STATE_BQ25120A BQ25120A_Set_LDO(uint8_t enable_ldo,uint32_t ldo_voltage,uint8_t MRRESET_cfg){
    ...
    if(enable_ldo == LDO_ENABLE){
        ...
        functions.GPIO_Write_Low(BQ_LSCTRL_Pin); //Disabled LDO Output over LSCTRL Pin

        HAL_Delay(100);

        BQ25120A_I2C_WRITE(ADDR_BQ25120A,LOADSW_LDO_CTRL_REG,ldo_data); //Enabled LDO output and set ldo output to ldo_voltage
        functions.GPIO_Write_High(BQ_LSCTRL_Pin); //Enabled LDO output
...
bool BQ25120A_Check_Valid_VinSource(uint32_t PG_pin_dio){

    BQ25120A_MR_Ctrl(APP_PUSH_BUTTON_CONFIG_2); //change PG pin to not reflects MR pin setting
    HAL_Delay(50);

    bool input_state = functions.GPIO_Read_Pin(PG_pin_dio);
...
void BQ25120A_I2C_WRITE(uint8_t slaveaddr, uint8_t regaddr, uint8_t data){
	...
	functions.GPIO_Write_High(BQ_CD_Pin); //to enable active battery mode for ı2c and disable charge
	HAL_Delay(10);
	functions.I2C_Write(slaveaddr,regaddr,1,(uint8_t *)&data,1);
	HAL_Delay(10);
	functions.GPIO_Write_Low(BQ_CD_Pin); //to disable active battery mode for enable charge
...
uint8_t BQ25120A_I2C_READ(uint8_t regaddr){
    ...
    functions.GPIO_Write_High(BQ_CD_Pin);//to enable active battery mode for ı2c and disable charge
    HAL_Delay(10);
    functions.I2C_Read(ADDR_BQ25120A,regaddr,1,(uint8_t *)&rx_data,1);
    HAL_Delay(10);
    functions.GPIO_Write_Low(BQ_CD_Pin); //to disable active battery mode for enable charge
